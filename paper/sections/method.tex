...
\end{equation}
Setting $P(\text{mismatch} \mid S_\mathrm{NEA}) = 1/2$ substantially
reduces the log-odds penalty for mismatches relative to the standard model
($\log\theta = \log 0.01$), making the NEA state largely agnostic to
disagreements.
Note that this is not strictly ``zero log-odds'' relative to the AMH state,
since the AMH emission uses $f^\mathrm{YRI}_i$ rather than $1/2$; however,
for sites with archaic minor allele (low $f^\mathrm{YRI}$), the AMH emission
at a mismatch site ($P = 1 - f^\mathrm{YRI} \approx 1$) also approximates 1,
so the relative contribution to the NEA/AMH log-odds ratio is negligible.
In practice this mode is appropriate whenever reference--donor divergence
means that mismatches cannot be reliably attributed to archaic absence.
